...
```bash
kubectl patch dclawapp quality --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
```bash
cd dclaw-quality/frontend && npm run dev
cd dclaw-quality/backend && uvicorn dclaw_quality.main:app --reload
```
